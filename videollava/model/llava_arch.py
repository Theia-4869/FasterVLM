...
class LlavaMetaForCausalLM(ABC):
    ...
    # FasterVLM
    def encode_videos(self, videos):  # (B, C, T, H, W)
        video_features, video_attentions = self.get_model().get_video_tower()(videos)  # (B, T, N, C), (B, T, M, N)

        # video_attentions = video_attentions.max(dim=2)[0] # (B, T, N) = (1, 8, 256)
        video_attentions = video_attentions.mean(dim=2) # (B, T, N) = (1, 8, 256)

        B, T, N, C = video_features.shape
        visual_token_num = self.get_visual_token_num() # L

        # prune visual tokens by attention scores
        token_indices = torch.topk(video_attentions, k=visual_token_num, dim=2)[1] # (B, T, L)
        token_indices = torch.sort(token_indices, dim=2)[0] # (B, T, L)

        token_indices_expand = token_indices.unsqueeze(-1).expand(-1, -1, -1, C)
        video_features = torch.gather(video_features, dim=2, index=token_indices_expand) # (B, T, L, C)

        video_features = self.get_model().mm_projector(video_features)  # (B, T, L, C)
        ...
        for image in tmp_image_features:
            if isinstance(image, list):
                for i in range(len(image)):
                    ...
                    visual_tokens += image[i].shape[0]
            else:
                new_tmp.append(image)
                visual_tokens += image.shape[0]
