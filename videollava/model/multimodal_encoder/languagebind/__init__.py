...
class LanguageBindVideoTower(nn.Module):
    ...
    def feature_select(self, video_forward_outs):
        ...
        video_attentions = video_forward_outs.attentions[self.select_layer]  # (B, T, H, N, N)
        if self.select_feature == 'patch':
            video_features = video_features[:, :, 1:]
            video_attentions = video_attentions[:, :, :, 0, 1:]
        elif self.select_feature == 'cls_patch':
            video_features = video_features
            video_attentions = video_attentions
        else:
            raise ValueError(f'Unexpected select feature: {self.select_feature}')
    ...
    def forward(self, videos):
        if type(videos) is list:
            video_features, video_attentions = [], []
            for video in videos:
                video_forward_out = self.video_tower(video.to(device=self.device, dtype=self.dtype).unsqueeze(0), output_attentions=True, output_hidden_states=True)
                video_feature, video_attention = self.feature_select(video_forward_outs)
                video_features.append(video_feature.to(videos.dtype))
                video_attentions.append(video_attention.to(videos.dtype))
        else:
            video_forward_outs = self.video_tower(videos.to(device=self.device, dtype=self.dtype), output_attentions=True, output_hidden_states=True)
            video_features, video_attentions = self.feature_select(video_forward_outs)
            video_features, video_attentions = video_features.to(videos.dtype), video_attentions.to(videos.dtype)
